...
export function registerInspectCommand(context: vscode.ExtensionContext): vscode.Disposable {
    return vscode.commands.registerCommand('fab-inspector.inspect', async () => {
        ...
        const rulesFile = await vscode.window.showInputBox({
            placeHolder: `Enter the name of the rules file within your ${rulesFolderName} folder (e.g., rules.json)`,
            ...
            validateInput: (value) => {
                ...
                try {
                    ValidationUtils.validateRulesFileName(value);
                    return null;
                } catch (error) {
        ...
        if (rulesFile && formats) {
            const rulesPath = path.join(workspaceFolder.uri.fsPath, rulesFolderName, rulesFile);
